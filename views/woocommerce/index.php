            ...
            <?= GridView::widget([
                ...
                'columns' => [
                    ...
                    [
                        ...
                        'contentOptions' => ['style' => 'vertical-align: middle;'],
                        'value' => function ($model) {
                            $displayKey = strlen($model->consumer_key) > 20 ? 
                                substr($model->consumer_key, 0, 10) . '...' . substr($model->consumer_key, -10) : 
                                $model->consumer_key;
                            
                            return Html::a(
                                '<i class="fas fa-key me-2 text-warning"></i>' . Html::encode($model->consumer_key),
                                ['view', 'id' => $model->id],
                                [
                                    'target' => '_blank',
                                    'class' => 'text-decoration-none',
                    ...
                    [
                        ...
                        'contentOptions' => ['style' => 'vertical-align: middle;'],
                        'value' => function ($model) {
                            $displaySecret = strlen($model->consumer_secret) > 20 ? 
                                substr($model->consumer_secret, 0, 10) . '...' . substr($model->consumer_secret, -10) : 
                                $model->consumer_secret;
                            
                            return Html::a(
                                '<i class="fas fa-shield-alt me-2 text-danger"></i>' . Html::encode($model->consumer_secret),
                                ['view', 'id' => $model->id],
                                [
                                    'target' => '_blank',
                                    'class' => 'text-decoration-none',
...
<style>
    .woocommerce-index .card {
        border-radius: 10px;
        overflow: hidden;
        box-shadow: 0 4px 15px rgba(0,0,0,0.08);
    }
    
    .woocommerce-index .table-hover tbody tr:hover {
        background-color: rgba(241, 172, 22, 0.05) !important;
        transform: translateY(-1px);
        box-shadow: 0 2px 8px rgba(0,0,0,0.1);
    }
    
    .woocommerce-index .table thead th {
        font-weight: 600;
    ...
    .woocommerce-index .table td, 
    .woocommerce-index .table th {
        padding: 1rem 1.25rem;
        vertical-align: middle;
    }
    
    .woocommerce-index .btn {
        border-radius: 20px;
    ...
    .woocommerce-index .btn:hover {
        transform: translateY(-2px);
        box-shadow: 0 4px 12px rgba(241, 172, 22, 0.3);
    }
    
    .woocommerce-index .card-footer {
        border-top: 1px solid #e9ecef !important;
    }
    
    .woocommerce-index .pagination .page-item.active .page-link {
        background-color: #f1ac16 !important;
        border-color: #f1ac16 !important;
        color: white !important;
    }
    
    .woocommerce-index .pagination .page-link:hover {
        background-color: #f1ac16 !important;
        border-color: #f1ac16 !important;
        color: white !important;
    }
...
$(document).ready(function() {
    ...
    $('.woocommerce-index .card').css('opacity', '0').animate({opacity: 1}, 500);
    
    // Effet de survol sur les lignes du tableau
    $('.woocommerce-index .table tbody tr').hover(
        function() {
            $(this).css({
                'transform': 'translateY(-2px)',
    ...
    $('.woocommerce-index .action-column a').hover(
        function() {

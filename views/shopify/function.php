...
{
    try {
        // Crée un objet DateTime à partir de la chaîne ISO 8601
        $date = new DateTime($isoDate, new DateTimeZone('UTC'));

        // Change le fuseau horaire en Europe/Paris
        $date->setTimezone(new DateTimeZone('Europe/Paris'));

        // Formate la date au format d/m/Y H:i:s
        return $date->format('d/m/Y H:i:s');
    } catch (Exception $e) {
        // Gérer les erreurs éventuelles
        return "Erreur de formatage de la date : " . $e->getMessage();
    }
}

function getId($string)
{
    // Divise la chaîne en segments en utilisant "/" comme délimiteur
